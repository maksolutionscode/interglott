...
import {
  readingTest2,
  READING_TEST_2_ALL_QUESTIONS,
  READING_TEST_2_DURATION_MIN,
  type Choice,
} from "@/data/tcfTef/readingTest2";
import { useProgress } from "@/contexts/UserProgressContext";
import { cn } from "@/lib/utils";

const STORAGE_KEY = "interglott-tcf-reading-test-2";
const PROGRESS_STORAGE_KEY = "interglott-tcf-progress";
const TOTAL_SECONDS = READING_TEST_2_DURATION_MIN * 60;
...
const TcfTefReadingTest2 = () => {
  ...
  const total = READING_TEST_2_ALL_QUESTIONS.length;
  ...
  const score = useMemo(() => {
    let correct = 0;
    for (const q of READING_TEST_2_ALL_QUESTIONS) {
  ...
  useEffect(() => {
    if (state.submitted && !xpAwardedRef.current) {
      ...
      try {
        ...
        if (!reading.includes(2)) {
          data.reading = [...reading, 2];
            ...
            {state.submitted ? (
              ...
              <>
                ...
                <span className="text-muted-foreground font-normal hidden sm:inline">
                  / {READING_TEST_2_DURATION_MIN}:00
                </span>
              </>
            )}
          </div>
            ...
            <div className="space-y-4">
              {section.questions.map((q) => {
                ...
                const qNumberDisplayed =
                  READING_TEST_2_ALL_QUESTIONS.findIndex((x) => x.id === q.id) + 1;
                return (
                  <div
                    key={q.id}
...
export default TcfTefReadingTest2;